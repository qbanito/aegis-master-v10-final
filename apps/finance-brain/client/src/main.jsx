...
function PromotionGate({botId,state}){const report=(state.infrastructure?.performance?.promotion?.strategies||[]).find(row=>row.strategyId===botId);if(!report)return null;const m=report.metrics||{},l=report.limits||{},blocked=report.dataQuality?.blocker;return <section className="promotionGate"><div className="promotionHead"><span>PERFORMANCE GATE</span><b className={report.stage==='PAPER_PROVEN'?'proven':report.stage.includes('RECOVERY')||report.stage.includes('HOLD')?'hold':'micro'}>{report.stage?.replaceAll('_',' ')}</b></div><div className="promotionMetrics"><div><small>CERRADAS REALES</small><strong>{m.closedRealQuotePaper||0}</strong></div><div><small>EXPECTANCY</small><strong className={Number(m.expectancyUsd)<0?'loss':''}>{Number(m.expectancyUsd)>=0?'+':''}{fmt(m.expectancyUsd)}</strong></div><div><small>PROFIT FACTOR</small><strong>{m.profitFactor??'—'}</strong></div><div><small>WIN RATE</small><strong>{pct(m.winRate)}</strong></div></div><div className="promotionLimits"><span>MICRO‑PAPER máx. <b>{fmt(l.maxNotionalUsd)}</b></span><span>{m.openPositions||0}/{l.maxOpenPositions||0} posiciones</span><span>mín. score {l.minBrainScore||'—'}</span></div>{blocked&&<div className="promotionBlock">{humanKey(blocked)}</div>}<small className="promotionNext">{report.nextGate}</small></section>}

function BotDrawer({bot,state,onClose,onRun,onToggle,onAnalyze,solanaWorker,solanaAnalysis,solanaBusy,onSetCryptoMode}){const m=BOT_META[bot.id]||BOT_META.liquidation,Icon=m.Icon,isSolana=['solana-radar','solana-meme-momentum'].includes(bot.id),isCrypto=isSolana||bot.id==='polygon-meme-momentum',crypto=state.infrastructure?.cryptoExecution?.[bot.id];return <div className="drawerOverlay" onClick={onClose}><aside className="drawer" onClick={e=>e.stopPropagation()} style={{'--accent':m.accent}}><button className="close" onClick={onClose}>×</button><div className="drawerHead"><span><Icon size={34}/></span><div><small>AGENTE {m.n}</small><h2>{m.label}</h2><em>● {bot.active?'ACTIVO':'PAUSADO'}</em></div></div><div className="drawerStats"><Metric label="PNL PAPER 24H" value={fmt(bot.pnl24h)} positive/><Metric label="OPORTUNIDADES" value={bot.opportunities}/><Metric label="NETWORK" value={bot.network||'MULTI'}/><Metric label="HEARTBEAT" value={bot.heartbeat?new Date(bot.heartbeat).toLocaleTimeString():'—'}/></div><PromotionGate botId={bot.id} state={state}/><ScannerInsight botId={bot.id} state={state}/>{isSolana&&<SolanaResearchPanel worker={solanaWorker} analysis={solanaAnalysis||state.infrastructure?.solanaWorker?.lastAnalysis} busy={solanaBusy} onAnalyze={onAnalyze}/>}<p>Los datos se leen en tiempo real; PAPER usa cotizaciones y ledger. LIVE solo se arma para revisión y exige simulación fresca, límites y firma manual de wallet.</p>{isCrypto&&<button className="drawerSecondary" onClick={()=>onSetCryptoMode(bot.id,crypto?.mode==='ARMED_LIVE'?'PAPER':'ARMED_LIVE')}><Shield size={16}/> {crypto?.mode==='ARMED_LIVE'?'VOLVER A PAPER':'ARMAR LIVE · FIRMA MANUAL'}</button>}<button className="drawerPrimary" onClick={onRun}><Radio size={16}/> EJECUTAR SCAN AHORA</button><button className="drawerSecondary" onClick={onToggle}><Power size={16}/> {bot.active?'PAUSAR AGENTE':'ACTIVAR AGENTE'}</button></aside></div>}
