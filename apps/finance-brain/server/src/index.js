...
app.post('/api/crypto-bots/:id/execution-mode',(req,res)=>{const id=String(req.params.id||''),supported=new Set(['solana-radar','solana-meme-momentum','polygon-meme-momentum']);if(!supported.has(id))return res.status(404).json({error:'CRYPTO_BOT_NOT_FOUND'});const mode=String(req.body?.mode||'').toUpperCase();if(!['PAPER','ARMED_LIVE'].includes(mode))return res.status(400).json({error:'INVALID_CRYPTO_EXECUTION_MODE'});const wallet=String(req.body?.wallet||'').trim()||null;if(mode==='ARMED_LIVE'&&!wallet)return res.status(409).json({error:'WALLET_REQUIRED_TO_ARM',message:'Conecta una wallet antes de armar LIVE.'});state.infrastructure.cryptoExecution[id]={mode,wallet,armedAt:mode==='ARMED_LIVE'?new Date().toISOString():null,live:false,requires:['fresh simulation','risk limits','manual wallet signature'],notice:'ARMED_LIVE prepara transacciones; este servidor no firma ni transmite trades reales.'};persist();broadcast();res.json(state.infrastructure.cryptoExecution[id]);});
