...
import {solanaWorkerClient} from './infrastructure/solana/solanaWorkerClient.js';
...
import {configureExecutionControl} from './core/executionControlService.js';
...
const operatorToken=String(process.env.AEGIS_OPERATOR_TOKEN||'');
const requireOperator=(req,res,next)=>{if(!operatorToken)return res.status(503).json({error:'OPERATOR_TOKEN_NOT_CONFIGURED',message:'Configura AEGIS_OPERATOR_TOKEN en Render antes de habilitar operaciones reales.'});if(String(req.get('x-aegis-operator-token')||'')!==operatorToken)return res.status(401).json({error:'OPERATOR_AUTH_REQUIRED'});next();};
const guardRealMode=(req,res,next)=>['REAL','LIVE','ARMED_LIVE'].includes(String(req.body?.mode||'').toUpperCase())?requireOperator(req,res,next):next();
...
const executionControl=configureExecutionControl({state,persist,broadcast});
...
state.infrastructure.cryptoExecution??={};
...
opportunityBus.on('raw-opportunity',async raw=>{
  ...
  const botExecutionMode=executionControl.bot(opportunity.strategyId)?.effectiveMode||'PAPER';
  const paperExecutionAllowed=botExecutionMode==='PAPER'&&PAPER_EXECUTE_WATCH&&(brain.decision==='CANDIDATE'||brain.decision==='WATCH');
  if((brain.decision==='CANDIDATE'||paperExecutionAllowed)&&risk.approved&&breaker.allowed){
    try{
      ...
      enriched.execution={id:crypto.randomUUID(),opportunityId:enriched.id,strategyId:enriched.strategyId,mode:botExecutionMode,status:'BLOCKED',error:error?.message||'EXECUTION_BLOCKED',createdAt:new Date().toISOString()};
      state.executions.unshift(enriched.execution);state.executions=state.executions.slice(0,250);
      journal.execution(enriched.execution);
    }
  }
...
app.get('/api/solana-worker/status',async(req,res)=>{try{const health=await solanaWorkerClient.health();state.infrastructure.solanaWorker={...solanaWorkerClient.status(),...health,online:Boolean(health.ok&&health.rpc?.online),lastCheckAt:new Date().toISOString()};persist();res.json(state.infrastructure.solanaWorker);}catch(error){const status={...solanaWorkerClient.status(),online:false,error:error?.message||'SOLANA_WORKER_UNAVAILABLE',lastCheckAt:new Date().toISOString()};state.infrastructure.solanaWorker={...state.infrastructure.solanaWorker,...status};persist();res.status(503).json(status);}});
app.post('/api/solana-worker/analyze',async(req,res)=>{try{const scan=(state.infrastructure.solana?.lastScans||[])[0]||state.infrastructure.marketBots?.results?.['solana-meme-momentum'];const candidates=Array.isArray(req.body?.candidates)?req.body.candidates:(scan?.launches||scan?.data?.candidates||[]);const analysis=await solanaWorkerClient.analyze(candidates);state.infrastructure.solanaWorker={...solanaWorkerClient.status(),online:true,lastAnalysis:analysis,lastCheckAt:new Date().toISOString()};persist();broadcast();res.json(analysis);}catch(error){res.status(503).json({ok:false,error:error?.message||'SOLANA_WORKER_ANALYSIS_FAILED'});}});
app.post('/api/solana-worker/simulate',async(req,res)=>{try{const transactionBase64=String(req.body?.transactionBase64||'');if(!transactionBase64)return res.status(400).json({error:'TRANSACTION_BASE64_REQUIRED'});res.json(await solanaWorkerClient.simulate(transactionBase64));}catch(error){res.status(422).json({ok:false,error:error?.message||'SOLANA_WORKER_SIMULATION_FAILED'});}});
app.get('/api/execution/control',(req,res)=>res.json(executionControl.snapshot()));
app.post('/api/execution/global-mode',guardRealMode,(req,res)=>{try{res.json(executionControl.setGlobal(req.body?.mode,{confirmReal:req.body?.confirmReal===true,wallets:req.body?.wallets||{}}));}catch(error){res.status(error.message==='REAL_MODE_CONFIRMATION_REQUIRED'||error.message==='GLOBAL_KILL_SWITCH'?409:400).json({error:error.message});}});
app.patch('/api/execution/bots/:id',guardRealMode,(req,res)=>{try{res.json(executionControl.setBot(String(req.params.id),req.body||{}));}catch(error){res.status(error.message==='BOT_NOT_FOUND'?404:error.message==='REAL_MODE_CONFIRMATION_REQUIRED'||error.message==='GLOBAL_KILL_SWITCH'?409:400).json({error:error.message});}});
app.get('/api/execution/pending-signatures',(req,res)=>res.json(executionControl.snapshot().pendingWalletSignatures));
app.get('/api/execution/pending-signatures/:id',requireOperator,(req,res)=>{try{res.json(executionControl.walletSignature(String(req.params.id)));}catch(error){res.status(404).json({error:error.message});}});
app.post('/api/execution/pending-signatures/:id/resolve',requireOperator,(req,res)=>{try{res.json(executionControl.resolveWalletSignature(String(req.params.id),req.body||{}));}catch(error){res.status(error.message==='SIGNING_REQUEST_NOT_FOUND'?404:400).json({error:error.message});}});
app.post('/api/crypto-bots/:id/execution-mode',guardRealMode,(req,res)=>{try{const mode=String(req.body?.mode||'').toUpperCase()==='ARMED_LIVE'?'REAL':req.body?.mode;const wallet=req.body?.wallet?{address:String(req.body.wallet),ecosystem:String(req.params.id).startsWith('solana')?'SOLANA':'EVM'}:undefined;res.json(executionControl.setBot(String(req.params.id),{mode,wallet,confirmReal:mode==='REAL'}));}catch(error){res.status(error.message==='BOT_NOT_FOUND'?404:400).json({error:error.message});}});
...
app.post('/api/system/mode',guardRealMode,(req,res)=>{try{const requested=String(req.body?.mode||'').toUpperCase(),mode=requested==='LIVE'?'REAL':requested;if(!['PAPER','REAL'].includes(mode))return res.status(400).json({error:'INVALID_MODE'});res.json(executionControl.setGlobal(mode,{confirmReal:req.body?.confirmReal===true,wallets:req.body?.wallets||{}}));}catch(error){res.status(error.message==='REAL_MODE_CONFIRMATION_REQUIRED'||error.message==='GLOBAL_KILL_SWITCH'?409:400).json({error:error.message});}});
...
app.get('/api/integrations/binance/account',requireOperator,async(req,res)=>{try{res.json(await binanceTrading.account(String(req.query.market||'spot')));}catch(error){res.status(400).json({error:error?.message||'BINANCE_ACCOUNT_ERROR'});}});
app.post('/api/integrations/binance/order',requireOperator,async(req,res)=>{try{const botId=String(req.body?.botId||'');const control=executionControl.bot(botId);if(!control||control.route.executionKind!=='CEX')return res.status(400).json({error:'CEX_BOT_REQUIRED'});if(control.mode!=='REAL')return res.status(409).json({error:'BOT_REAL_MODE_REQUIRED'});if(req.body?.confirmReal!==true)return res.status(409).json({error:'REAL_ORDER_CONFIRMATION_REQUIRED'});const {botId:_botId,confirmReal:_confirmReal,...order}=req.body||{};res.json(await binanceTrading.order(order));}catch(error){res.status(400).json({error:error?.message||'BINANCE_ORDER_ERROR'});}});
...
app.post('/api/integrations/solana/swap/submit',requireOperator,async(req,res)=>{try{res.json(await solanaTrading.submitSignedTransaction(String(req.body?.signedTransactionBase64||'')));}catch(error){res.status(400).json({error:error?.message||'SOLANA_SWAP_SUBMIT_ERROR'});}});
...
app.post('/api/integrations/polymarket/proposals/:id/approve',requireOperator,async(req,res)=>{try{res.json(await polymarketTrading.approveProposal(req.params.id));}catch(error){res.status(400).json({error:error?.message||'POLYMARKET_ORDER_ERROR'});}});
