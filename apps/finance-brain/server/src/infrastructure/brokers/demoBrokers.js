...
export class PublicTradFiDataConnector{
  ...
  async fxPrices(instruments=csv(process.env.FX_PAIRS||'EUR_USD,USD_JPY,GBP_USD,AUD_USD')){
    let rates={},observedAt=isoNow(),provider='ExchangeRate API public reference';
    try{const body=await fetchJson(`${this.fxBase}/latest/USD`,{timeoutMs:8000,retries:1,errorPrefix:'PUBLIC_FX'});rates=body?.rates||{};observedAt=body?.time_last_update_utc||observedAt;}
    catch(primaryError){
      const currencies=[...new Set(instruments.flatMap(item=>String(item).toUpperCase().split('_')).filter(code=>code&&code!=='USD'))];
      const query=new URLSearchParams({base:'USD',symbols:currencies.join(',')});const fallback=await fetchJson(`https://api.frankfurter.dev/v1/latest?${query}`,{timeoutMs:8000,retries:1,errorPrefix:'FRANKFURTER_FX'});
      rates=fallback?.rates||{};observedAt=fallback?.date||observedAt;provider='Frankfurter public reference';
    }
    const prices=instruments.map(instrument=>{const [base,quote]=String(instrument).toUpperCase().split('_');const usdBase=base==='USD'?1:1/Number(rates[base]||0);const value=quote==='USD'?usdBase:Number(rates[quote]||0)*usdBase;const spread=Math.max(value*0.0003,0.00001);return {instrument,bid:value-spread/2,ask:value+spread/2,mid:value,observedAt,provider,indicative:true};}).filter(item=>item.mid>0);
    return {provider,prices,base:'USD',observedAt,dataQuality:'REFERENCE_RATE_NOT_TICK'};
  }
...
export class PolygonPaperQuoteConnector{
  ...
  status(){return {id:'polygon-paper-quotes',provider:'0x Swap API',mode:'SERVER_QUOTE_BROWSER_SIGN',configured:this.configured(),online:null,live:false,liveExecutionReady:this.configured(),chainId:this.chainId,base:this.base,capabilities:['polygon-token-quotes','paper-entry-exit-pricing','browser-signed-swap-preparation'],warning:'El servidor prepara calldata; MetaMask revisa, firma y transmite desde el navegador.'};}
  async quote({inputToken,outputToken,amount,inputDecimals=6,outputDecimals=18,slippageBps=100}={}){
    ...
    const url=new URL(`${this.base}/swap/allowance-holder/price`);for(const [key,value] of Object.entries({chainId:this.chainId,sellToken:inputToken,buyToken:outputToken,sellAmount:amount,slippageBps}))url.searchParams.set(key,String(value));const headers={accept:'application/json','0x-api-key':this.apiKey,'0x-version':'v2'};const body=await fetchJson(url,{headers,timeoutMs:10000,retries:2,errorPrefix:'POLYGON_0X_QUOTE'});if(!body?.buyAmount)throw new Error(body?.validationErrors?.[0]?.reason||body?.reason||'POLYGON_QUOTE_EMPTY');return {...body,inputToken,outputToken,inputDecimals,outputDecimals,observedAt:isoNow(),provider:'0x Swap API Polygon'};
  }
  async prepareSwap({inputToken,outputToken,amount,taker,slippageBps=100}={}){
    if(!this.configured())throw new Error('POLYGON_QUOTE_API_KEY_MISSING');if(!inputToken||!outputToken||!amount||!taker)throw new Error('POLYGON_SWAP_PARAMETERS_REQUIRED');
    const url=new URL(`${this.base}/swap/allowance-holder/quote`);for(const [key,value] of Object.entries({chainId:this.chainId,sellToken:inputToken,buyToken:outputToken,sellAmount:amount,taker,slippageBps}))url.searchParams.set(key,String(value));
    const headers={accept:'application/json','0x-api-key':this.apiKey,'0x-version':'v2'};const body=await fetchJson(url,{headers,timeoutMs:12000,retries:1,errorPrefix:'POLYGON_0X_SWAP'});if(!body?.transaction?.to||!body?.transaction?.data)throw new Error(body?.validationErrors?.[0]?.reason||body?.reason||'POLYGON_SWAP_TRANSACTION_MISSING');
    const spender=body?.issues?.allowance?.spender||body.allowanceTarget||null;return{provider:'0x Swap API v2',chainId:this.chainId,transaction:body.transaction,approval:spender?{token:inputToken,spender,amount:String(amount),exactAmount:true}:null,buyAmount:body.buyAmount,sellAmount:String(amount),liquidityAvailable:body.liquidityAvailable!==false,simulationIncomplete:Boolean(body?.issues?.simulationIncomplete),preparedAt:isoNow()};
  }
}
