...
export class RealMarketPaperBroker{
  constructor({state,persist,onClose,solanaTrading}={}){
    ...
    this.trailingActivationPct=envNum('PAPER_TRAILING_ACTIVATION_PCT',8,1);
    this.trailingStopPct=envNum('PAPER_TRAILING_STOP_PCT',5,1);
  }
  status(){return {mode:'REAL_MARKET_PAPER',modelFallback:this.modelFallback,modelMaxTradesPerHour:this.modelMaxTradesPerHour,live:false,feeBps:this.feeBps,maxSpreadBps:this.maxSpreadBps,maxOpenPositions:this.maxOpenPositions,holdMs:this.holdMs,markIntervalMs:this.markIntervalMs,trailingActivationPct:this.trailingActivationPct,trailingStopPct:this.trailingStopPct,openPositions:this.state.paperLedger?.openPositions?.length||0};}
  ...
  async open(opportunity,simulation){
    ...
    const plan=opportunity.metadata?.paperPlan||{};const stopLossPct=Math.max(0,num(plan.stopLossPct||opportunity.metadata?.stopLossPct||0));const takeProfitPct=Math.max(0,num(plan.takeProfitPct||opportunity.metadata?.takeProfitPct||0));const maxLossUsd=Math.max(.01,num(opportunity.metadata?.maxLossUsd||this.state.risk?.maxLossPerTradeUsd||50));
    const position={id:crypto.randomUUID(),opportunityId:opportunity.id,strategyId:opportunity.strategyId,asset:opportunity.asset,network:opportunity.network,direction,provider:quote.provider,quantity:fill.quantity,entryPrice:fill.price,entryNotional:fill.filledNotional,entryFeeUsd:fee,entrySlippageUsd:slippage,maxLossUsd,stopLossPct,takeProfitPct,trailingActivationPct:this.trailingActivationPct,trailingStopPct:this.trailingStopPct,peakPrice:fill.price,trailingStopPrice:null,openedAt:now(),holdUntil:Date.now()+this.holdMs,markPrice:fill.price,unrealizedPnlUsd:0};
