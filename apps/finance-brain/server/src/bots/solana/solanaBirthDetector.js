...
export class SolanaBirthDetector{
  constructor({rpc,programs,onBirth=()=>{},onReorg=()=>{},onStatus=()=>{},WebSocketImpl=WebSocket}={}){
    ...
    this.wsUrl=String(process.env.SOLANA_WS_URL||deriveWsUrl(rpc?.url)).trim();
    ...
    this.ws=null;this.timer=null;this.reconnectTimer=null;this.requestId=1;this.subscriptions=new Map();this.seenSignatures=new Set();this.pending=new Set();this.confirmationTimers=new Set();this.reconnectAttempt=0;
    this.metrics={events:0,mints:0,duplicates:0,reorgs:0,processed:0,failed:0,lastBirthAt:null,lastError:null};
  }
  ...
  status(){return {enabled:this.enabled,configured:Boolean(this.enabled&&this.wsUrl&&this.programs.length),connected:Boolean(this.ws&&this.ws.readyState===this.WebSocketImpl.OPEN),subscriptions:this.subscriptions.size,queued:this.pending.size,events:this.metrics.events,mints:this.metrics.mints,duplicates:this.metrics.duplicates,reorgs:this.metrics.reorgs,failed:this.metrics.failed,lastBirthAt:this.metrics.lastBirthAt,lastError:this.metrics.lastError,commitment:this.commitment,mode:'READ_ONLY',live:false};}
  emitStatus(){this.onStatus(this.status());}
  start(){if(this.timer||!this.enabled||!this.wsUrl||!this.programs.length){this.emitStatus();return this.status();}this.connect();this.timer=setInterval(()=>{if(!this.ws||this.ws.readyState!==this.WebSocketImpl.OPEN)this.connect();},Math.max(5000,Number(process.env.SOLANA_RADAR_WS_HEARTBEAT_MS||15000)));this.timer.unref?.();return this.status();}
  stop(){clearInterval(this.timer);this.timer=null;clearTimeout(this.reconnectTimer);this.reconnectTimer=null;for(const timer of this.confirmationTimers)clearTimeout(timer);this.confirmationTimers.clear();this.subscriptions.clear();if(this.ws){this.ws.removeAllListeners?.();this.ws.close?.();}this.ws=null;this.emitStatus();}
  connect(){if(!this.enabled||!this.wsUrl||!this.programs.length||this.ws?.readyState===this.WebSocketImpl.OPEN||this.ws?.readyState===this.WebSocketImpl.CONNECTING)return;try{const ws=new this.WebSocketImpl(this.wsUrl);this.ws=ws;ws.on('open',()=>{this.reconnectAttempt=0;this.subscribe();this.emitStatus();});ws.on('message',data=>this.handleMessage(data));ws.on('error',error=>{this.metrics.lastError=error?.message||'SOLANA_WS_ERROR';this.emitStatus();});ws.on('close',()=>{this.subscriptions.clear();this.ws=null;this.emitStatus();if(this.timer){const delay=Math.min(30000,500*Math.pow(2,this.reconnectAttempt++));this.reconnectTimer=setTimeout(()=>this.connect(),delay);this.reconnectTimer.unref?.();}});}catch(error){this.metrics.lastError=error?.message||'SOLANA_WS_CONNECT_ERROR';this.emitStatus();}}
  subscribe(){for(const program of this.programs){const id=this.requestId++;this.subscriptions.set(id,program);this.ws.send(JSON.stringify({jsonrpc:'2.0',id,method:'logsSubscribe',params:[{mentions:[program]},{commitment:this.commitment}] }));}}
  ...
  enqueue(event){if(this.seenSignatures.has(event.signature)){this.metrics.duplicates++;return;}this.seenSignatures.add(event.signature);if(this.seenSignatures.size>10000)this.seenSignatures=new Set([...this.seenSignatures].slice(-5000));this.pending.add(event.signature);this.process(event).catch(error=>{this.metrics.failed++;this.metrics.lastError=error?.message||'SOLANA_BIRTH_PROCESS_ERROR';}).finally(()=>{this.pending.delete(event.signature);this.emitStatus();});}
