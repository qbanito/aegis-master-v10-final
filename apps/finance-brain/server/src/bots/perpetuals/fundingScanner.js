...
export class FundingScanner{
  constructor({marketData,bus,onStatus,onScan}){
    ...
    this.paperNotionalUsd=Math.max(100,Number(process.env.PERP_PAPER_NOTIONAL_USD||2000));
  }
  config(){return {provider:'Binance USDⓈ-M Futures public market data',symbols:this.symbols,scanEveryMs:this.scanEveryMs,minAbsFundingBps:this.minAbsFundingBps,minAbsBasisBps:this.minAbsBasisBps,paperNotionalUsd:this.paperNotionalUsd,configured:this.symbols.length>0};}
  async analyze(symbol){
    ...
    const oiContracts=Number(oi.openInterest||0);const absFunding=Math.abs(fundingBps),absBasis=Math.abs(basisBps);const triggered=absFunding>=this.minAbsFundingBps||absBasis>=this.minAbsBasisBps;
